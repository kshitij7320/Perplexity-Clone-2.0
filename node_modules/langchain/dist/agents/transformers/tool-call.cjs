...
function isHeadlessToolInterruptError(message, toolCallId) {
	try {
...
function createToolCallTransformer(path) {
	return () => {
		const toolCallsLog = _langchain_langgraph.StreamChannel.local();
				...
				if (event.method === "tools") {
					...
					if (pending) {
						if (data.event === "tool-finished") {
							...
							if (isHeadlessToolInterruptError(message, toolCallId)) return true;
